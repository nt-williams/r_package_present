...
There are two ways of doing this: imports and depends. `imports` and `depends` both will install packages that aren't already installed on a system, their only difference is that `depends` will also load the package into the environment. For our package we will just use `imports` as we are accessing the `glue()`  using `::`. 

To include *glue* as an import in DESCRIPTION we can run `use_package(glue)`. If we open DESCRIPTION we should now see an `imports` field for *glue*.
...
Up to this point we have been informally checking the performance of our function. While this works for quick checks as you write functions it is not scalable. For example, at some point in your package's timeline you will release updates, how would you ensure that these updates don't create issues in functionality. It is because of this we should formally write unit tests (a unit test is just a tests that evaluates individual components of software). To do this we will use the *testhat* package. 

Per usual, *devtools* has a helper function for setting this up. Run `use_test()` in the console. A new folder should be created in your directory called `tests` which should contain another folder, `testhat`.
